    ...
    class SavetToCsvFile
    {
        public static void ExtractDataToCSV(DataTable dtTab)
        {
            ...
            if (sfd.ShowDialog() == true)
            {
                ...
                var dbtblDescription = new DbTblDescription();
                dbtblDescription.listOfFieldDescriptions = new List<DbTblFldDesc>();

                dbtblDescription.tableName = sfd.SafeFileName;
                for (int i = 0; i < dtTab.Columns.Count; i++)
                {
                    var dbtblFldDesc = new DbTblFldDesc();
                    dbtblFldDesc.FldName = dtTab.Columns[i].ColumnName;
                    dbtblFldDesc.FldType = dtTab.Columns[i].DataType.ToString();
                    dbtblFldDesc.FldLen = dtTab.Columns[i].MaxLength;
                    dbtblDescription.listOfFieldDescriptions.Add(dbtblFldDesc);
                }

                string jsStr = new JavaScriptSerializer().Serialize(dbtblDescription);
                string filePath = Path.Combine(Path.GetDirectoryName(sfd.FileName), Path.GetFileNameWithoutExtension(sfd.FileName) + ".json");

                File.WriteAllText(filePath, jsStr);
            }
